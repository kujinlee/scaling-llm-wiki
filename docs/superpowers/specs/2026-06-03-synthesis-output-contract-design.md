...
| **Sentinel present but its path fails the concept-path shape** (e.g. an uppercase/underscore slug → zero splits) | **Raises `ValueError` → retry** (adversarial-review finding). A loose `^===WIKI-FILE:` detector in the no-split branch distinguishes "model botched a sentinel path" (retry, loud) from a genuine old-style JSON reply (fall back) — preventing the body from being silently returned as `files:[]`. Cannot false-fire on valid JSON (escaped `\n` means `===WIKI-FILE:` never starts a physical line) |
